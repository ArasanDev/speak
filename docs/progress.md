...
-6. **Loop #38 (2026-06-29) — agentic-loop hardening + SM-0.** Four things landed via the full issue→branch→worker→review→PR loop:
   ...
   - **#40 SM-0 eval harness LANDED** (#5 → PR #6): `make eval` runs golden `spoken→expected` fixtures (14, 2/profile, seeded from `profile-system-prompts.md`) through each profile and prints a per-profile pass/score + p50/p95 table. `SpeakCore/Eval/EvalScoring.swift` (pure Jaccard correctness + declarative format checks + percentiles, all thresholds `[decision]`-tagged); `EvalHarnessTests` runs scoring+plumbing+**regression-detection** in normal `make test` via a deterministic `MockCleaner`, with **live Foundation Models scoring gated behind `SPEAK_EVAL=1`** (reuses the test host's proven FM access — NOT a CLI tool, which is unverified). Gates: build ✅ / test 542,1-skip,0-fail ✅ / lint 0-serious ✅ / moat 7/7 ✅. **Orchestration note:** Haiku worker (builder-qa) built it from a locked brief; orchestrator review caught + fixed before commit a `%s`-with-Swift-`String` table-writer bug (live-path only, invisible to `make test`), dead vars, unwired `XCTAttachment`s, and 3 force-unwrap lint errors. **SM-1 (measured FM-limits study) is now unblocked.**
