...
| Agent-aware dictation fallback | Provider plugins and application-specific integrations `[verified]` | Faithful technical prompt profile works in any focused field without a provider adapter | **MATCH** | v0.1 (V01-0) | Same spoken task remains faithful and imperative in configured agent surfaces; ordinary prose remains natural elsewhere; no direct-session claim without a receipt |

Post-v0 human-agent interface benchmarks are defined in
`specs/agent-voice-bridge.md`: owned request outcomes, session identity,
delivery acknowledgement/misrouting, response latency, and interruption noise.
They do not retroactively expand the v0 dictation MATCH/BEAT gate.
