...
**The Pet — working name "Pip."** A creature whose body is a *live waveform*:
...
Pip's form is ours: bars-as-body means every animation is a true signal
readout, never decoration. A VU meter that became a pet.

Everything else in the app stays quiet so Pip can be the memorable thing.
...
- One orchestrated moment: capture-start — Pip (or HUD) ignites `onAir`,
  bars jump to live levels within 100ms. This is the product's handshake;
  it must feel instant.

## 5. Pip — the Pet (component spec)
...
- Right-click menu: Start/Stop Dictation · Open speak · Hide Pip
  (session) · Disable Pip (setting). Menu is the escape hatch; keep it short.
...
- **FE-1 (now): tokens + Pip.** `Speak/App/DesignSystem/` (Color/Font/Motion
...
- **Horizon (design-noted, NOT built):** conversation threads per agent
  ...
  model — speak as the local Slack for your agents). Pip's hover lozenge is
