...
| # | Task | Owner | Tier |
|---|------|-------|------|
| 2.1 | **Pluggable cleanup-model UI** — Ollama + MLX as user-selectable cleaners behind `LLMCleaning`; FM default. ✅ MERGED — UI+seam ship; Ollama/MLX are `isAvailable==false` stubs (no networking in build, moat 7/7); real localhost client → future `SpeakLLM` module (v0.1). | builder-cleanup + builder-app | Sonnet |
| 2.2 | **Richer cleanup** — tone/style + snippets + custom dictionary at cleanup time. ✅ MERGED — style modes + snippet expander were already wired; net-new = custom-dictionary "preserve spellings" clause in the FM prompt (`CleanupMode.styled(...,customVocabulary:)`, 50-term cap, empty=baseline). Per-app formatting rules NOT done (defer to Wave 3 if wanted). | builder-cleanup | Sonnet |
| 2.4 | **Latency / metrics view** — stop→paste (raw + cleanup) per `benchmark.md §7`. ✅ MERGED — monotonic instrumentation + median/p95 in Insights, budget-colored; raw/cleanup partition via `cleanupSeconds==0.0` sentinel; +20 unit tests. Plumbing only — real numbers need a live dictation run (Human-Gate Track). | builder-app + builder-engine | Sonnet |
| 2.3 | ✅ MERGED — `speak --start/--stop/--status` over CFMessagePort (`com.speak.app.cli`) behind `CLITransport`; not-running→exit 1; idempotency icon-gate; moat 7/7 (CLI/ now scanned). Caveats→Phase-2: double-start race + tests-are-reimplementation. **[decision] Transport = CFMessagePort** (named same-user mach port, synchronous request/reply returning JSON) — needs no hardened-runtime entitlement at P11 notarize (vs NSXPC mach-service registration), and `--status` needs a structured reply (rules out DistributedNotifications/URL-scheme fire-and-forget). Put it behind a thin `CLITransport` protocol — transport is the *reversible* part, don't over-invest. **The real design risk (pin these, not the byte format):** (a) `--start` when app isn't running → launch-then-start or error? (b) externally-triggered start/stop must be **idempotent + main-actor-safe** (`--start` while already recording, `--stop` while idle, invoked from an IPC callback not the hotkey path); (c) CLI target + SpeakCore linking + Homebrew symlink rpath in `project.yml`. Wire contract: JSON request `{"cmd":"start|stop|status"}` → JSON reply `{"ok":bool,...}` (status adds `state`/`binding`). | builder-infra/release + builder-engine | Sonnet |
