...
| 2.4 | **Latency / metrics view** — surface measured stop→paste (raw + cleanup) per `benchmark.md §7`, feeding the P13/P14 numbers. **DO 2.4 BEFORE 2.3** — it produces the v0 ship-gate latency numbers (Human-Gate Track), higher leverage than a v1 CLI. Timing instrumentation touches `CaptureSession` → must land after 1.4. | builder-app + builder-engine | Sonnet |
