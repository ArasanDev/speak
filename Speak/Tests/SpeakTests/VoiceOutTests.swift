...
final class MockSpeechSynthesizerTests: XCTestCase {
    ...
    /// the caller having to call `stop()` themselves. The second call still
    /// follows the normal contract from `testSpeakReportsSpeakingUntilStopped`
    /// (stays in flight until an explicit `stop()`) — it is not auto-resolved.
    func testSecondSpeakInterruptsFirst() async {
        ...
        let secondTask = Task { await mock.speak("second", locale: Locale(identifier: "en-US")) }
        await firstTask.value   // must have resolved once interrupted by the second call

        await mock.stop()       // resolve the now-current (second) utterance
        await secondTask.value

        let texts = await mock.calls.map(\.text)
        XCTAssertEqual(texts, ["first", "second"], "both speak() calls must be recorded, in order")
        let speaking = await mock.isSpeaking
        XCTAssertFalse(speaking, "stop() resolves the second utterance and clears isSpeaking")
    }
}
