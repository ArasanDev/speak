...
- Requires one OS permission: **Accessibility** only. The tap location (`.cghidEventTap`) is Accessibility-gated regardless of tap options. The tap must not be installed until Accessibility is granted; attempting without it silently fails or crashes.
